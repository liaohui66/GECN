...
class SE3InvariantGraphEncoder(nn.Module):
    def __init__(self, num_atom_types: int = 100, 
                 embedding_dim_scalar: int = 120, 
        ...
        self.node_embedding = nn.Embedding(num_atom_types, actual_atom_feature_dim_from_data)
    ...
    def forward(self, data: Data) -> Tuple[torch.Tensor, o3.Irreps]:
        ...
        # --- Checkpoint 1.1: After input data check ---
        if torch.isnan(pos).any():
            raise RuntimeError("NaN detected in input data.pos!")
        if torch.isnan(node_features).any():
            raise RuntimeError("NaN detected in input data.x (node_features)!")

        current_node_features = self.lin(node_features)
